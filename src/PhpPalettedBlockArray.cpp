...
static bool palette_data_from_string(zend_string* paletteZstr, std::vector<Block>& palette, zend_class_entry* exception_ce) {
	if (!PaletteBytes::isValidByteSize(ZSTR_LEN(paletteZstr))) {
		zend_throw_exception_ex(exception_ce, 0, "palette length in bytes must be a multiple of %zu, but have %zu bytes", PaletteBytes::ENTRY_SIZE, ZSTR_LEN(paletteZstr));
		return false;
	}

	palette = PaletteBytes::decode<Block>(ZSTR_VAL(paletteZstr), ZSTR_LEN(paletteZstr) / PaletteBytes::ENTRY_SIZE);
...
static bool paletted_block_array_from_data(zval *return_value, zend_long bitsPerBlock, zend_string *wordArrayZstr, HashTable *paletteHt, zend_string *paletteZstr) {
	...
	if (paletteHt != nullptr) {
		if (!palette_data_from_array(paletteHt, palette, paletted_block_array_load_exception_entry)) {
			return false;
		}
	} else if (!palette_data_from_string(paletteZstr, palette, paletted_block_array_load_exception_entry)) {
		return false;
	}
...
static void paletted_block_array_get_palette_bytes(zval *object, zval *return_value) {
	...
	zend_string* result = zend_string_alloc(PaletteBytes::byteSizeOf(palette.size()), 0);
	PaletteBytes::encode<Block>(palette, ZSTR_VAL(result));
...
PALETTED_BLOCK_ARRAY_METHOD(fromData) {
	...
	HashTable *paletteHt = nullptr;
	zend_string *paletteZstr = nullptr;
